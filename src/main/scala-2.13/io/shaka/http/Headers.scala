...
case class Headers(headers: List[Header]) {
  ...
  def map[A](f: Header => A): List[A] = headers.map(f)
...
object Headers {
  ...
  def toHeaders(rawHeaders: java.util.Map[String, java.util.List[String]]): Headers =
    Headers(
      rawHeaders.asScala.toList.flatMap { case (k, v) =>
        v.asScala.map((httpHeader(k), _)).toList
      }
    )
}
