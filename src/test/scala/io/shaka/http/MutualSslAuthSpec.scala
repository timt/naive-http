...
import io.shaka.http.Https.{TrustServersByTrustStore, UseKeyStore}
...
class MutualSslAuthSpec extends FunSuite with BeforeAndAfterEach {
  test("Client can connect to server doing mutual SSL auth") {
    val server = HttpServer.https(io.shaka.http.HttpsConfig(
    ...
    try {
      val response = Http.http(GET(s"https://127.0.0.1:${server.port}/foo"))(httpsConfig = Some(io.shaka.http.Https.HttpsConfig(
